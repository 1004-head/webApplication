...
function readAllCustomers() {
    ...
    $statement = runQuery($pdo, "select * from Customers order by FirstName;", null);
    return $statement;
}
function readSelectCustomers($lastName) {
    ...
    $statement = runQuery($pdo, "select * from Customers where LastName='{$lastName}';", null);
    return $statement;
}
...
function readImprints() {
    ...
    // imprints를 배너에 프린트
    $pdo = setConnectionInfo(array(DBCONNECTION, DBUSER, DBPASS));
    $statement = runQuery($pdo, "select * from Imprints;", null);
    return $statement;
}
